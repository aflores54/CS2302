...
#Last Edit 5/7, Build Maze Path using a Adjacency List, Breadth First Search, and Depth First Search
...
def Create_maze(walls, maze_rows, maze_cols, M):#Creates maze by removing walls through 
    ...
    if N > 0:#This prints a text statement if path 0 to the upper right corner may or may not exist
        print("A single path to destination may not exist")
    if N == 0:
        print("A single path to destination exists")
    if N < 0:#This is for when NumSets reached 1 instead of N, so we remove more walls to match M
        N = N*(-1)
        for i in range(M):
